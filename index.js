...
const main = async () => {
  ...
  let privateKeys;
  try {
    const fileContent = fs.readFileSync('privateKeys.json', 'utf-8');
    if (!fileContent) {
    ...
    console.error('Failed to read privateKeys.json:'.red, error);
    process.exit(1);
  }
  ...
  for (const privateKey of privateKeys) {
    ...
    await printSenderBalance(); // Await to prevent async issues
